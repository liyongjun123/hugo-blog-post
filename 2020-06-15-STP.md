...
根端口（Root Port），即去王根桥路径最近的端口，这个最近的衡量是靠 Root Path Cost 来判定的。Path Cost 的计算，是每当一个端口收到一个 BPDU 后，会在该 BPDU 所指示的 Path Cost 上加上该端口的 Port Path Cost（这是可以人为配置的）。比较累计 Root Path Cost 最小的端口就是根端口，如果两条路径开销相同，那么就选择 BID 较小的。
