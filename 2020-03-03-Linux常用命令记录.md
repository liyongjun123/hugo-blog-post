...
`screen -ls` 列出当前所有的作业  
`screen -r 作业序号` 回到某个作业中  
`screen -d 作业名称` 远程结束某个作业  
`screen -d -r 作业名称` 结束当前的这个作业并回到某个作业中  
`Ctrl + a  k` 杀掉当前窗口  
`Ctrl + a  d` 保存进程并退出作业(程序在screen中继续运行，screen -ls 可查看)  
`Ctrl + a  n`  切换到下一个窗口  
[链接](https://www.cnblogs.com/zhang-zi-yi/p/10795346.html)  
